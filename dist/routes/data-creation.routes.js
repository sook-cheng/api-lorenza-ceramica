...
exports.dataCreationRoutes = dataCreationRoutes;
...
async function dataCreationRoutes(fastify) {
    ...
    fastify.get("/create-categories", async (request, reply) => {
        ...
        for (const p of data_1.categories) {
    ...
    fastify.get("/create-mainProductsSideNavs", async (request, reply) => {
        ...
        for (const p of data_1.mainProductsSideNavs) {
    ...
    fastify.get("/create-subProductsSideNavs", async (request, reply) => {
        let sql = "INSERT INTO productsSideNavs (name,path,tableName,sequence,mainSideNavId) VALUES";
        for (const p of data_1.subProductsSideNavs) {
            sql += `('${p.name}','${p.path}','${p.tableName}',${p.sequence},'${p.mainSideNavId}'),`;
        }
        sql = sql.replaceAll("'null'", "null");
        return sql.substring(0, sql.length - 1) + ";";
    ...
    fastify.get("/create-finishes", async (request, reply) => {
        ...
        for (const p of data_1.finishes) {
            sql += `('${p.name}','${p.value}'),`;
        }
        sql = sql.replaceAll("'null'", "null");
        return sql.substring(0, sql.length - 1) + ";";
    ...
    fastify.get("/create-tileTypes", async (request, reply) => {
        ...
        for (const p of data_1.tileTypes) {
            sql += `('${p.name}','${p.value}'),`;
        }
        sql = sql.replaceAll("'null'", "null");
        return sql.substring(0, sql.length - 1) + ";";
    ...
    fastify.get("/create-productsTags", async (request, reply) => {
        let sql;
        for (const p of data_1.addProductsTags1) {
    ...
    fastify.get("/create-productsTagsWithoutCode", async (request, reply) => {
        let sql;
        for (const p of data_1.productsTagsWithoutCode) {
            const tags = p.tags.split(", ");
            for (const t of tags) {
                sql += "\nINSERT INTO productsTags (productId,tagId) ";
    ...
    fastify.get("/create-productsTagsSales", async (request, reply) => {
        let sql;
        for (const p of data_1.productsTagsSales) {
            const tags = p.tags.split(", ");
            for (const t of tags) {
                if (!t)
                    continue;
